...
router.post("/verify-product", isAuth, isUser, async function (req, res, next) {
...
router.get("/product-verify", async function (req, res, next) {
  const scanCount = await History.countDocuments({ user: req.user });
  res.status(200).send(scanCount);
});

router.get("/history", isAuth, isUser, async function (req, res, next) {
